...
- 팀 활동 요구사항
    - [X] 먼저 해당 데이터셋을 통해서 어떤 비즈니스 가치를 만들 수 있을지에 대해 토론하세요. (먼저 여러 개의 아이디어를 만들고 그중에 하나를 고르는 게 효율적입니다.)
      - [ideation](#76--개인적으로-ideation-해오기)
      - [개별 Prototype 구현](#간단-프로토-타입-개별-구현-구현-가능성-검증용)
      - [최종 팀 아이디어 선정 과정](#google-form-아이디어-평가-진행-및-최종-팀-아이디어-선정)
      - [아이디어 고도화](#아이디어-고도화)
    - [X] 데이터셋은 웹 스크레이핑을 통해 직접 만든 다음, word cloud를 만들어 봅시다.
      - [Extract 오케스트레이터](/team_wiki/W2/W2M5/scrape_signal.py)
      - [HTML/HTTP 파싱 코드(개별 채용 파싱))](/team_wiki/W2/W2M5/scrape_html.py)
      - [Greeting 스크래핑](/team_wiki/W2/W2M5/ats_greeting.py)
    - [X] 데이터셋을 만들 때 어떤 작업들이 추가적으로 필요할까요?
      - [데이터셋을 만들 때 어떤 작업들이 추가적으로 필요할까요?](#데이터셋을-만들-때-어떤-작업들이-추가적으로-필요할까요)
    - [X] prototyping에는 최소 1,000개 이상의 데이터를 사용하세요.
        <details>
        <summary>수집된 양(4406건) 확인</summary>
...
- 피드백
    ...
    - 데이터 품질의 측면에서, 이쁘지 않다.를 이쁘로 형태소가 갈라져 나오면, 오해의 여지가 있을 수 있다. (길원)
...
---
#### Google Form 아이디어 평가 진행 및 최종 팀 아이디어 선정
- 객관적인 비즈니스 선정을 위해 다음과 같은 항목 설정 후 1-5점 평가 진행
    - Use-Case
        - 1 : 있으면 좋은 수준 (없어도 됨)
        - 3 : 불편하지만 우회 방법으로 가능
        - 5 : 지금 당장 해결책을 적극적으로 찾는 중

        - 구체성
            - 1 : 기업이 사용 가능하다 정도의 막연
            - 3 : 특정 산업/ 직군 좁혀지지만, 사용 시나리오 추상적
            - 5 : 특정 팀의 특정 담당자가 특정 업무에 쓰는 장면이 image화

        - 지불 의사 (기대효과)
            - 1 : 무료여도 사용할지 의문 (가치가 없다 느낌)
            - 3 : 저렴하면 사용하지만, 예산을 쓸 필요 없음 (가치가 크지 않다 느낌)
            - 5 : 문제 해결 가치가 명확해서 높은 가격에도 구매/도입할 근거가 있음.
...
>> 마케팅팀보다, `IT 교육 업체에서 더 필요로 할 것 같다.` 의견으로 제품 `피봇팅`
...
---
#### 한계 논의
> 채용 인원이 모두 다르고, 영세한 기업/악덕 기업일 수록 채용이 잦다. 
...
- 이민하
    - 기업 도메인 별로 사용하는 기술이 달라서 도메인의 차이로 워드 클라우드가 달라질 수 있다.
        - 도메인별로도 분석이 가능하게 하자
- 전길원
    - 단순 언급 수가 아닌 회사 수로 집계하여, Word Cloud만들 때 Custom 가중치로 넣자.
        - 한국은 공고 수가 미국에 비해 적어서 일부 기업이 10% 20%를 차지하는 경우가 있음
    - 기업 도메인을 다양하게 조사해서 특정 업종에서 자주 쓰는 기술만 보이는걸 피해야 할듯
        - 세그먼트별로 나눠 총 62개 회사 공고 조사
    - 최신 분기에 언급이 늘어나고 있는게 더 잘 보여야 부트캠프에서 의사결정을 할 수 있을 것 같다.
        - 이 또한 Custom Weight로 1분기 전마다 *0.6을 해서 지금 뜨는 기술을 강조하여 보여주자.
    - 특정 직무에서만 쓰는 테크 스택을 보여주는게 부트캠프 커리큘럼이나 마케팅에서 의사결정에 도움이 될 것 같다.
        - TF-IDF 구조를 이용하여, 전체 테크 스택 대비 특정 직무에서 언급되는 양을 점유율비로 계산하여 Custom Weight로 WordCloud 표현
- 문종민
