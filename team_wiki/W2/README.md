...
### 2차 회고
- Time Boxing
  - 시간이 모자랄 수 있으므로 50-60분은 남겨둠
  1. 각자 개선할 점 찾기 (17:30 - 17:40)
    <details>
    <summary>길원</summary>
    
    1. 채용 공고 자체도 경력과 신입을 구분하기
      - 신입이 할 수 있는 것과 경력이 할 수 있는 것은 크게 차이남
      - 신입 (0-2년차) / 주니어(3-7년차 미만) / 시니어 (7년차 이상)으로 구분하여 수집하기

    2. 채용 공고 수집 길이를 늘리기. 현재는 긴 시계열을 확인하기 어려울 듯 함. 
      - 갈수록 이러한 추세로 테크 스택이 넘어가고 있다를 보여주도록
        - ex, 10년전에 비해 클라우드 활용이 중요해짐. 이런거

    3. 나눌 때 기술 분류까지 고려하여 나누기 (언어 / 인프라 / DB / 라이브러리 / 프레임워크,...)

    4. Actionable Data를 보여주기 위해 다음 요소 추가
      - UI로 현재 커리큘럼 입력
      - 현재 커리에 있는 것 제외

    5. User Flow 
      - 현재 서비스하는 커리큘럼 입력 (수기/이미지/URL 등)
      - 현재 기준 신입 / 주니어 /시니어의 직무별/ 기간별 워드 클라우드를 선택하여 확인 가능 (radio 버튼 등)
        - 이때, 현재 커리에 포함된 요소는 제거
    </details>
    ---
    <details>
    <summary>은정</summary>
    ...
    1. 의사결정 목적 구체화
      - 실제 의사결정에 어떻게 쓰는지를 조금 더 생각해보자. 현재 결과는 기술 트렌드를 보여주는 수준 
      - 부트캠프 및 교육기관의 커리큘럼 추가, 유지, 축소 판단을 지원하는 프로젝트로 목적을 구체화할 필요가 있음
      - 단순 기술 언급 순위가 아니라 교육과정에 반영할 수 있는 기준을 제공해야 함
    2. ‘채용 수요’ 표현의 범위 조정
      - 현재 데이터로는 실제 채용 인원이나 채용 성공 여부를 알 수 없음
      - 기술 언급 횟수 ≠ 실제 시장 수요
      - ‘채용공고 내 기술 언급 빈도’
    3. 기술 사전의 정확도 검증
      - React와 React Native처럼 기술명이 일부 겹치는 경우 중복 집계될 수 있음
      - AWS와 Amazon Web Services처럼 같은 기술이 다른 이름으로 표현될 수 있음
      - 동의어와 상하위 기술 관계를 정리해 하나의 표준 기술명으로 통합할 필요가 있음
      - 일부 공고를 직접 확인하여 기술 추출 결과의 정확도를 검증할 필요가 있음
    4. 워드클라우드 외 분석 결과 추가
      - 워드클라우드는 기술의 상대적인 크기를 확인하기에는 적합하지만 정확한 수치와 변화를 비교하기 어려움
      - 기술별 언급 회사 수, 직무별 순위, 분기별 증감률과 같은 정량적 결과를 함께 제공할 필요가 있음
      - 기술의 성장, 유지, 감소 흐름을 확인할 수 있는 시계열 그래프를 추가할 필요가 있음
    5. 기술 스택 분류 체계 개선
      - Python과 Airflow처럼 성격이 다른 기술이 동일한 기준으로 집계되고 있음
      - 기술을 프로그래밍 언어, 프레임워크, 데이터베이스, 클라우드, 데이터 처리 도구 등으로 분류
      - 전체 기술 순위와 함께 동일한 기술 범주 내 순위를 별도로 제공할 필요가 있음
    6. 분석 기간 확대
      - 현재 약 3개 분기의 채용공고를 분석해 단기적인 변화만 확인할 수 있음
      - 분석 기간이 짧아 일시적인 채용 증가와 지속적인 기술 흐름을 구분하기 어려움
      - 분석 기간을 2~3년 이상으로 확대하고 분기별 증감 추이를 비교할 필요가 있음
      - 장기간의 데이터를 바탕으로 기술별 성장, 유지, 감소 흐름을 구분할 필요가 있음
    7. 채용공고와 교육 대상 간 차이
      - 현재 데이터에서는 신입, 주니어, 경력직 공고를 구분하지 않음
      - 채용공고에는 일정 수준 이상의 경력과 실무 경험을 요구하는 공고가 많이 포함될 수 있음
      - 부트캠프와 온라인 강의는 입문자와 취업 준비생을 주요 대상으로 하는 경우가 많음
      - 채용공고에 자주 등장하는 기술이 입문자가 우선적으로 학습해야 하는 기술과 일치하지 않을 수 있음
      - 신입, 주니어, 경력직 공고를 구분하여 기술 요구 차이를 분석할 필요가 있음
      - 교육과정 의사결정에는 신입·주니어 공고를 중심으로 한 별도의 분석 결과를 제공할 필요가 있음
    </details>
    ---
    <details> 
    <summary>민하</summary>

    - 실제 우리가 해결해야할 문제가 워드 클라우드로 보여주기 좋은 지표인지 아니다. 
      - 분기별 변화를 보여주려면 시계열적인 특성이 더 잘 보이게 프로토타이핑 

    - 타깃이 정말 필요로 하는 지표가 없다.
      - 현재 타깃이 커리큘럼으로 채택한 기술은 꾸준히 잘쓰이고 있고,
      - 반면 커리큘럼에 들어있지 않지만 최근 갑자기 많이 보이는 기술을 찾아서 그 정보를 제공해야 한다.
        - 단순히 jd 데이터 뿐만이 아니라 실제 시장에서 그 기술이 많이 쓰이고 있는지 근거를 보여주고
        - “타깃의 타깃” (ex. 우리는 비기너 대상으로 강의를 한다.) 라면 해당 기술이 비기너가 알아야하고 난이도가 적절한 기술인지도 같이 보여줘야한다.
    ...
    - 테크스택 난이도별 분류 필요
      - 실무자 레벨에서 사용하는건지 초보자 레벨에서 사용하는건지

    - 판매 대상에 대한 조사와 데이터 필요
      - 고객사에서 이미 교육과정 운영중인 테크스택인지 등

    - 트렌드 파악을 위해 시계열 성격 강화 필요
      - 2년치정도 분기별로 하면 트렌드 보일듯

    - 구체적인 수치 분석 필요
      - 시간에 따른 증감량이나 순위 등 비교

    - 워드클라우드를 메인으로 하기보다 보조로 하는게 나을듯
      - 워드클라우드 자체는 사실 별 쓸모없음

    - 가시성 개선 필요할듯
      - 테라폼의 경우 크고 중요함에도 불구하고 글씨가 연해서 잘 안보임

    - 좀 더 데이터를 다양하게 세분화
      - 분야별/깊이별/유형별/시기별/지역별/회사규모별 등
      - 전체 언급량/중복제거언급량/증감량/신규언급량 등

    - 최종적인 분석 결론 필요
      - 분야별 언급 top5/ 증가추세 top5 등
      - ~하기 때문에 ~~ 테크스택 교육과정을 신설하면/비중을 늘리면 경쟁력 강화에 도움 될 것이다.
    </details>

    ---
  2. 찾은 개선할 점 의논 (17:40 - 18:30)
    - 비즈니스 개선할 점
      1. 해결하려는 의사결정을 구체화하기
        - 우리 마인드에서 구체화해서 특정 하나를 잡고 구체화
        - 도메인에 대한 생각을 덜 했다.
          - 데이터를 보여주고, 뭔가 actionable data를 뽑아내는 것을 안한 느낌
        - 신입 부트캠프로 제한
          - 테크스택도 깔끔해지고, 수집도 쉽고, 채용 공고가 별로 없다. (2년차 수집)
      2. 고객이 정말 필요로 하는 지표가 없다.
        - 어떤 것을 볼 수 있을까?
          1. 전체 언급량 : 직관적인 절대값
          2. 중복 제거 언급량 (회사당 1개) : 회사가 잦은 채용이 있는 경우 뻥튀기 될 수 있음
          3. 증감량 (분기별 차이량) 
          4. 신규 언급량 (회사 기준으로 신규 언급량) : 안 쓰던 회사들이 도입을 했다.
          5. 해당 기간동안 점유율이 많이 증가한 기술스텍 : 과거 대비 시장 점유율이 늘었다.
          6. 지금 커리큘럼에 들어있는 것 여부 표시
      3. 기술 언급 횟수가 실제 시장 수요가 맞는지 아닌지 검증 필요
        - 시장 수요 없는데 기술 언급을 하는 경우
          - 채용 공고는 여는데 안 닫아
            - 증명할 방법이 있을까?
            - 데이터 수집 소스를 바꿔볼까?
        - 이건 너무 어려운 문제 이걸 고민했다는거 칭찬해줘야함.
      4. 분석 기간 확대
        - 최대 조사 (MAX:10Y)
          - Datetime으로 수집 가능하면 변경
    - 기술적 개선할 점
      1. 기술 스택 분류 체계 개선 (언어 / 인프라 / DB / 라이브러리 / 프레임워크,...)
      2. 기술 스택 중복 검증 개선
      3. 데이터 표시 방식 변경
        ![](objective_graph.png)
        - 조사 : 전체 공고 별 기술 스택의 점유율 
        - 표시 
          1. 점유율 상승률
          2. 그냥 전체 점유율
  3. 구현할 내용 선정 (18:30-18:40)
    1. 수집한 기간 늘리기(10Y)
      - Datetime을 시계열로 바꾸고, 보여줄 수 있는 방식(X축)을 분기별 월별 2개로 만들기
    2. 데이터 표시 방식 변경
    3. 수기로 커리큘럼 입력해서 구분 표시 처리
    4. 기술 스택 분류 체계 개선
----
