...
따라서 데이터 분석을 진행할 때는 두 변수 간의 관계만 볼 것이 아니라, 데이터를 다각적으로 바라보는 과정이 필요합니다. 아울러 mtcars는 표본이 32대에 불과하기에 실제 비즈니스 의사결정에 활용하려면 더 많은 최신 차량 데이터를 확보해 검증하는 과정이 추가로 필요합니다.
![](Weight_vs_MPG.png)
